...
export default function CheckoutPage() {
  ...
  const [fulfilmentChannel, setFulfilmentChannel] = useState<StoreFulfilmentMode>('delivery')
  const [pickupLeadMinutes, setPickupLeadMinutes] = useState<number | null>(null)
  const [pickupWindowMinutes, setPickupWindowMinutes] = useState<number | null>(null)
  const [pickupTimeLabel, setPickupTimeLabel] = useState<string | undefined>(undefined)
  ...
  const storePickupActive = pickupOffered && fulfilmentChannel === 'collect'
  const showDeliveryMethods = addressChecked && !storePickupActive
  ...
  const courierShipping =
    storePickupActive || !hasCourierGuyItems
      ? 0
      : hasGumtreeItems && !hasOtherCourierItems && gumtreeFulfillmentMethod === 'collect'
        ? 0
        : Number(dynamicRates[deliveryMethod] || 0)
  const courierShippingForDisplay = hasCourierGuyItems && addressChecked && !storePickupActive ? courierShipping : 0
  ...
  useEffect(() => {
    setFulfilmentChannel(getStoreFulfilmentMode())
    let cancelled = false
    ;(async () => {
      try {
        const raw = await newsApi.siteSettings.list()
        const map = rowsToMap(unwrapSiteSettingsList(raw))
        if (cancelled) return
        setPickupOffered(isTruthySetting(map.pickup_enabled))
        setPickupLeadMinutes(coerceSiteNumber(map.pickup_lead_time_minutes))
        setPickupWindowMinutes(coerceSiteNumber(map.pickup_window_minutes))
        const ptl = coerceSiteString(map.pickup_time_label).trim()
        setPickupTimeLabel(ptl || undefined)
      } catch {
  ...
  useEffect(() => {
    if (!storePickupActive || !hasCourierGuyItems) return
  ...
  useEffect(() => {
    ...
    if (hasCourierGuyItems && !addressChecked && !storePickupActive) return
    const effectiveMethod = storePickupActive ? 'collect' : deliveryMethod
    const payload: { delivery_method: string; pudo_pickup_point?: PudoLocation } = {
      delivery_method: effectiveMethod,
    }
    if (deliveryMethod === 'pudo' && selectedPudoLocation && !storePickupActive) {
      payload.pudo_pickup_point = selectedPudoLocation
    }
    if (hasCourierGuyItems) {
      ;(payload as any).shipping_override = storePickupActive ? 0 : courierShipping
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (hasCourierGuyItems && !addressChecked && !storePickupActive) {
    ...
    try {
      const effectiveCartMethod = storePickupActive ? 'collect' : deliveryMethod
      const cartUpdatePayload: { delivery_method: string; pudo_pickup_point?: PudoLocation } = {
        delivery_method: effectiveCartMethod,
      }
      if (deliveryMethod === 'pudo' && selectedPudoLocation && !storePickupActive) {
        cartUpdatePayload.pudo_pickup_point = selectedPudoLocation
      }
      ;(cartUpdatePayload as any).shipping_override = storePickupActive
        ? 0
        : hasCourierGuyItems
          ? dynamicRates[deliveryMethod] || 0
          : 0
      ...
      } else if (storePickupActive) {
        const addr = (company.contact.address || '').trim()
        const firstLine = addr.split(',')[0]?.trim() || addr || 'Store pickup'
        shippingAddress = {
          businessName: formData.business_name,
          address: addr || firstLine,
          street_address: firstLine,
          suburb: '',
          city: '',
          province: '',
          postal_code: '',
      ...
      const primaryDeliveryMethod = storePickupActive
        ? 'collect'
      ...
      const slotIso = typeof window !== 'undefined' ? getPickupSlotIso() : ''
      const slotNote =
        storePickupActive && slotIso
          ? `Pickup slot: ${new Date(slotIso).toISOString()}`
          : ''
      const mergedNotes = [formData.customer_notes?.trim(), slotNote].filter(Boolean).join('\n')
      ...
      const orderPayload: Record<string, unknown> = {
        ...
        courier_guy_shipping: storePickupActive ? 0 : courierShipping,
        ...
        <form onSubmit={handleSubmit} data-cy="checkout-form" className="min-w-0">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              {/* Shipping Address + verification — hidden for store pickup */}
              {storePickupActive ? (
                <div className="card p-4 md:p-6 min-w-0 overflow-hidden">
                  ...
                  {company.contact.address ? (
                    <p className="text-sm text-text-muted mb-4 whitespace-pre-line">{company.contact.address}</p>
                  ) : (
                    <p className="text-sm text-text-muted mb-4">Pickup location confirmed after payment.</p>
                  )}
                  <PickupTimePicker
                    leadMinutes={pickupLeadMinutes}
                    windowMinutes={pickupWindowMinutes}
