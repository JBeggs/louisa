...
import {
  getMinQuantity,
  getResolvedSpiceLevel,
  getStockQuantity,
  isBundleProduct,
  isGumtreeProduct,
  isTimedProduct,
} from '@/lib/product-utils'
import SpiceLevel from '@/components/products/SpiceLevel'
...
export default async function ProductPage({ params }: ProductPageProps) {
  ...
  const spiceLevel = getResolvedSpiceLevel(product)
          ...
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-16 items-start">
            ...
            <div className="space-y-8">
              <div>
                ...
                {product.allergens && String(product.allergens).trim() && (
                  <p className="text-sm text-text-muted mb-6 border border-border-default rounded-card p-3 bg-surface-raised">
                    {String(product.allergens).trim()}
                  </p>
                )}
              </div>
